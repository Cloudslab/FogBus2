import csv
from dependencies import Application, Task, Dependency, loadDependencies
...
class EdgeWeightLookup:
    ...
    def __init__(self, edgeWeights: List[EdgeWeight]):
        self.__edgeWeightsDict = defaultdict(lambda: defaultdict(lambda: defaultdict(EdgeWeight)))

        for edgeWeight in edgeWeights:
            self.__edgeWeightsDict[edgeWeight.app.name][edgeWeight.source][edgeWeight.destination] = edgeWeight
...
class EdgeWeights:
    ...
    def __init__(self, edgeWeights: List[EdgeWeight]):
        ...
        self.__edgeWeightsLookup: EdgeWeightLookup = EdgeWeightLookup(edgeWeights)
...
class TaskWeightLookup:
    ...
    def __init__(self, taskWeights: List[TaskWeight]):
        ...
        for taskWeight in taskWeights:
            self.__taskWeightsDict[taskWeight.app.name][taskWeight.task.name] = taskWeight
...
class TaskWeights(Weight):
    ...
    def __init__(self, taskWeights: List[TaskWeight]):
        ...
        self.__edgeWeightsLookup: TaskWeightLookup = TaskWeightLookup(taskWeights)
